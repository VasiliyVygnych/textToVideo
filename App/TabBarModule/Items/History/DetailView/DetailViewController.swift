...
class DetailViewController: UIViewController {
    ...
    private func setupeButton() {
        backButton.setBackgroundImage(HomeConstants.HomeImage.arrowLeft,
                                      for: .normal)
        shareButton.setBackgroundImage(SettingsConstants.SettingsImages.shareImages,
                                       for: .normal)
...
//                        if let url = self?.viewModel?.createDocumentURL(url) {  // save to files
//                            self?.saveVideoToFiles(url)
...
//                    }
   ...
   private func accessPermission() {
    ...
    private func errorSave() {
        ...
        shareButton.setBackgroundImage(UIImage(systemName: "xmark.circle"),
                                                for: .normal)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5 ) { [weak self] in
            self?.shareButton.setBackgroundImage(SettingsConstants.SettingsImages.shareImages,
                                                 for: .normal)
        }
    }
    private func successSave() {
        ...
        shareButton.setBackgroundImage(UIImage(systemName: "checkmark.circle"),
                                                for: .normal)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5 ) { [weak self] in
            self?.shareButton.setBackgroundImage(SettingsConstants.SettingsImages.shareImages,
                                                 for: .normal)
    ...
    private func addConstraints() {
        ...
        saveIndicator.snp.makeConstraints { make in
            make.centerY.centerX.equalTo(shareButton)
        }
